<script lang="ts" generics="MODE extends 'CREATE' | 'UPDATE'">
    ...
    const create_item = async (label: string | null): Promise<number> => {
        ...
        const suggested = suggested_items.find((i) => i.name === label && i.id < 0);
        if (suggested) {
            toast.info(`Using suggested item: ${label}`);
            if (!selectable_items.some((i) => i.value === suggested.id)) {
                selectable_items = [...selectable_items, { label, value: suggested.id }];
            }
            update_item_detail(suggested.id);
    ...
    <Dialog.Content class="sm:max-w-[425px]" escapeKeydownBehavior="close" interactOutsideBehavior="ignore">
        ...
        <div class="flex flex-col gap-2">
            <div class="flex flex-row items-center gap-2">
                ...
                <!-- <TagSelector bind:value={tag_ids} tags={selectable_tags} suggested_tags_map={suggested_tags_map} disabled={disabled} /> -->
            </div>
