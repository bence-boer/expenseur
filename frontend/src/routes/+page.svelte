<script lang="ts">
    ...
    import { format_currency_short, format_date, format_number } from '$lib/utils';
    ...
    const fetch_data = (period: Period, average?: Frequency): Promise<Data> =>
        Promise.all([
            service.get_expenses_by_category({ start_date: period.start.toString(), end_date: period.end.toString() }),
            service.get_expenses_by_category({ start_date: period.previous_start.toString(), end_date: period.previous_end.toString() })
